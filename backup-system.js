...
if (import.meta.url === `file://${process.argv[1]}`) {
  const backupSystem = new FlexibleBackupSystem();
  
  // 統計表示
  console.log('📊 バックアップシステム統計:');
  console.log(backupSystem.getBackupStats());
  
  // 手動バックアップテスト
  console.log('\n🧪 手動バックアップテスト:');
  backupSystem.createBackup('test');
  
  // ファイル監視開始
  console.log('\n👀 ファイル監視を開始します...');
  console.log('   Ctrl+C で停止');
  
  const watcher = backupSystem.startWatching();
